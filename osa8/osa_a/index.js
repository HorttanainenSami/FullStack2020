...
const resolvers = {
  ...
  Author: {
    bookCount: (root) => {
      return (
        books.reduce((sum, acc) => acc.author === root.name ? sum + 1 : sum, 0)
      )
    }
  },
  Mutation: {
    ...
    editAuthor: (root, args) => {
      console.log(args)
      const matching = authors.find(author => author.name === args.name)
      if (!matching) {
        return null
      }
      const newAuthor = { ...matching, born: args.setBornTo}
